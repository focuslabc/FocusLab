...
import { X, UserPlus, UserCheck, MessageCircle, Map } from 'lucide-react';
...
export function UserProfileModal({ userId, currentUserId, onClose, onSendFriendRequest, onOpenDM, friendshipStatus = 'none' }: UserProfileModalProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} onClick={e => e.stopPropagation()}
        ...
        <div className="flex items-center justify-center gap-2 mb-6 bg-black/30 border border-white/5 rounded-xl p-3">
          ...
          <span className="text-xs text-zinc-600">({streak} dias)</span>
        </div>
