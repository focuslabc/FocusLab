...
import { Users, Search, Send, ArrowLeft, UserPlus, UserCheck, X, Loader2, MessageCircle } from 'lucide-react';
  ...
  return (
    <div className="h-full w-full p-4 sm:p-6 lg:p-12 overflow-y-auto">
      <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 flex items-center gap-3">
        <Users className="w-6 h-6 sm:w-8 sm:h-8 text-red-500" /> FriLabs
      </h1>
      <p className="text-zinc-500 font-medium mb-6 text-sm sm:text-base">Encontre amigos e converse.</p>
            ...
            {searchResults.map(u => {
              ...
              return (
                <div key={u.user_id} className="flex items-center gap-3 p-3 hover:bg-white/5 transition-colors">
                  ...
                  {status === 'accepted' ? (
                    <button onClick={() => setActiveDM(u.user_id)} className="px-3 py-1.5 bg-blue-900/30 text-blue-400 rounded-lg text-xs font-bold"><MessageCircle className="w-3 h-3 inline mr-1" />Chat</button>
                  ) : status === 'pending' ? (
                    <span className="text-yellow-500 text-xs font-bold">Pendente</span>
                  ) : (
                    <button onClick={() => sendRequest(u.user_id)} className="px-3 py-1.5 bg-red-900/30 text-red-400 rounded-lg text-xs font-bold"><UserPlus className="w-3 h-3 inline mr-1" />Seguir</button>
                  )}
                </div>
              );
            })}
      ...
      {pendingRequests.length > 0 && (
        <div className="mb-6">
          <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-widest mb-3">Solicitações Pendentes</h3>
      ...
      {loading ? <Loader2 className="w-8 h-8 text-red-500 animate-spin mx-auto" /> :
        friends.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {friends.map(f => (
              <FriendCard key={f.id} friendUserId={getFriendUserId(f)} onOpenDM={(uid) => setActiveDM(uid)} onRemove={() => removeFriend(f.id)} />
            ))}
          </div>
        )}
    </div>
  ...
  return (
    <div className="h-full w-full flex flex-col">
      <div className="px-4 sm:px-6 py-4 border-b border-white/5 flex items-center gap-3">
        ...
        {otherProfile?.avatar_url ? <img src={otherProfile.avatar_url} alt="" className="w-8 h-8 rounded-full object-cover" /> : <div className="w-8 h-8 rounded-full bg-zinc-800" />}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        ...
        {messages.map((msg: any) => (
          <div key={msg.id} className={`flex ${msg.sender_id === userId ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] px-3 py-2 rounded-xl text-sm ${msg.sender_id === userId ? 'bg-red-900/30 text-white rounded-br-sm' : 'bg-zinc-800 text-zinc-200 rounded-bl-sm'}`}>
              {msg.content}
            </div>
          </div>
        ))}
